...
public class AddNewReservation {
	...
	public AddNewReservation(String UID, String date, String time, String depart, String destination, 
	...
	public CreateReservation getInstance() throws FillException{
		...
		if(dateR.equals("")) {
			if(numOfCollege+numOfNormal+numOfChildren+numOfSenior+numOfChallenged >10) {
				throw new FillException("票數不得超過10張");
			}
		} else {
			if(numOfCollege+numOfNormal+numOfChildren+numOfSenior+numOfChallenged >5) {
				throw new FillException("來回票數各別不得超過5張");
			}
			if (dateR.compareTo(date)<0) {
				throw new FillException("去程不可晚於回程");
			}
			if (dateR.compareTo(date)==0) {
			...
			if (dateCheck(dateR) == false) {
				throw new FillException("訂票失敗 " + this.dateR.substring(0,4)+"/"+this.dateR.substring(4,6)+"/"+this.dateR.substring(6,8) + "未能預約" );
			}
		}
		if (dateCheck(date) == false) {
			throw new FillException("訂票失敗 " + this.date.substring(0,4)+"/"+this.date.substring(4,6)+"/"+this.date.substring(6,8) + "未能預約" );
		}
